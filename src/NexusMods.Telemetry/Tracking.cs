...
using EventTuple = ValueTuple<EventDefinition, EventMetadata, ulong>;
...
public static class Tracking
{
    ...
    /// Create an event.
    /// </summary>
    public static void CreateEvent(EventDefinition definition, EventMetadata metadata)
    {
        EventSender sender = null!;
        sender.AddEvent(definition, metadata);
    }
}
...
internal class EventSender
{
    ...
    private readonly EventTuple[] _insertRingBuffer = new EventTuple[Limit];
    private readonly EventTuple[] _sortedReadingCopy = new EventTuple[Limit];
    ...
    /// </summary>
    internal void AddEvent(EventDefinition definition, EventMetadata metadata)
    {
        ...
        var tuple = new EventTuple(definition, metadata, id);
    ...
    internal async ValueTask Run()
    {
        ...
        PrepareRequest(writer, span, userId);
        var s = Encoding.UTF8.GetString(writer.WrittenSpan);

        await SendEvents(writer.WrittenMemory, _httpClient, CancellationToken.None);
    }

    private static void PrepareRequest(IBufferWriter<byte> writer, ReadOnlySpan<EventTuple> events, Optional<UserId> userId)
    {
    ...
    private static void SerializeEvent(IBufferWriter<byte> writer, EventTuple tuple, Optional<UserId> userId)
    {
    ...
    /// </summary>
    private static ReadOnlySpan<EventTuple> PrepareSpan(EventTuple[] input, ulong highestSeenId)
    {
        ...
        if (sliceStartIndex >= Limit) return ReadOnlySpan<EventTuple>.Empty;
