...
public class MainWindowViewModel : AViewModel<IMainWindowViewModel>, IMainWindowViewModel
{
    ...
    {
        var game = _serviceProvider.GetServices<ILocatableGame>().First(x => x.NexusModsGameId == nexusModsGameId);

        if (workspaceController.ActiveWorkspace.Context is LoadoutContext existingLoadoutContext && IsCorrectLoadoutForGame(existingLoadoutContext.LoadoutId, game.GameId))
            return (existingLoadoutContext.LoadoutId, workspaceController.ActiveWorkspaceId);

        var loadoutId = GetActiveLoadoutForGame(game.GameId);
